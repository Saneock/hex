 ...
 * false.
 */
class Session extends Object implements \IteratorAggregate, \ArrayAccess, \Countable
{
    /**
    ...
    public function init()
    {
        ...
        if ($this->getIsActive()) {
            Hex::$app->getLogger()->warning("Session is already started");
    ...
    public function open()
    {
        ...
        if ($this->getIsActive()) {
            Hex::$app->getLogger()->info('Session started');
            $this->updateFlashCounters();
        } else {
            $error = error_get_last();
            $message = isset($error['message']) ? $error['message'] : 'Failed to start session.';
            Hex::$app->getLogger()->error($message);
        }
    }

    /**
     * Registers session handler.
     * @throws \Hex\Base\InvalidConfigException
     */
    protected function registerSessionHandler()
    {
        if ($this->handler !== null) {
            if (!is_object($this->handler)) {
                $this->handler = Hex::createObject($this->handler);
            }
